...
import { FileFilter, GitignoreParser, IncludeExcludeGitignoreParser, ContentContextGitignoreParser } from '../utils/fileFilters';
import { WORKSPACE_FILES } from '../constants/extensionStorageFolderItems';
    ...
    private async getWorkspaceFilters(workspaceUri: vscode.Uri): Promise<IncludeExcludeGitignoreParser> {
        ...
        if (!this.workspaceFilters.has(workspaceKey)) {
            const structureFilter = await IncludeExcludeGitignoreParser.create(
                workspaceUri,
                WORKSPACE_FILES.INCLUDE_STRUCTURE_GITIGNORE,
                WORKSPACE_FILES.EXCLUDE_STRUCTURE_GITIGNORE
            );
